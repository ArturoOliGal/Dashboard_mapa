    ...
    <script>
        const setSVGFill = () => {
            ...
            if (prefersDarkScheme) {
                svgIcon.setAttribute('fill', 'black');  // Relleno del SVG negro para el tema oscuro
            } else {
                svgIcon.setAttribute('fill', 'white');  // Relleno del SVG blanco para el tema claro
            }
        };
        window.addEventListener('load', setSVGFill);
        window.matchMedia("(prefers-color-scheme: dark)").addEventListener('change', setSVGFill);
    </script>    
     <footer style ="text-align: center">
         <a href="#top">
             <button style="position: fixed; bottom: 10px; center: 10px; background-color: transparent; border: border: 2px solid white; color: white; padding: 10px 10px; text-align: center; text-decoration: none; font-size: 16px; border-radius: 50%;">
